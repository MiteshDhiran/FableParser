...
let getCharToStringExeptSep exceptCharArry = 
    let seqArr = Array.toSeq exceptCharArry
    let validCharsFn  (ch:char) = not (Seq.contains ch seqArr)
    many (satisfy validCharsFn)

let charsExcept exceptChars =
    let exceptCharsSeq = Array.toSeq exceptChars
    let charNot ch carray =
        not (Seq.contains ch carray)    
    many (satisfy (fun ch -> charNot ch exceptCharsSeq))
...
//let res = run fieldParser "123aA~BBB"
